...
def publish_prediction(predicted_label, confidence):
    MQTT_HOST = "localhost"  # Use the IP address of the MQTT broker if it's not running on the Raspberry Pi
    MQTT_TOPIC = "prediction/result"

    # Format the message
    message = f"Predicted: {predicted_label} with confidence {confidence:.2f}"

    # Publish the message
    publish.single(MQTT_TOPIC, message, hostname=MQTT_HOST)
